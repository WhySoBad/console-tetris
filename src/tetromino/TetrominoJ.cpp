...
void TetrominoJ::rotate() {
    ...
    points[3].setX((step == 2 || step == 3) ? x : step == 1 ? x + 2 : x + 1);
    points[3].setY((step == 3 || step == 4) ? y : step == 1 ? y + 1 : y + 2);
}

TetrominoJ::TetrominoJ(int startX, int startY): Tetris(startX, startY, j) {
